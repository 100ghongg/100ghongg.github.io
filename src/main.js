...
vBind({
    ...
    methods: {
        ...
        atRemain(){
            // ghong
            return loc(`accelerated_time`) + (global.genes['timeaccel'] ? ' (+' + global.settings.timeAccelerationFactorBonus + 'x)' : '');
        },
        pause(){
...
function midLoop(){
    ...
    if (global.race.species === 'protoplasm'){
        ...
        if (global.arpa['sequence'] && global.arpa.sequence.on && gene_sequence){
            ...
            // ghong. gene sequence boost.
            if (global.genes['seqboost']) {
                global.arpa.sequence.time -= (global.arpa.sequence.boost ? labs * 2 : labs) * (1.0 + global.genes['seqboost'] * 0.5);
            }
            else {
